...
TTL/reason/author) — it changes on its own rhythm (TTL sweeps) and is only present while at least one
block is active. Both are the only files node-stats writes into a dynamic dir, and it only ever
