...
func (h *DiskHandler) HandleDiskStats(c *gin.Context) {
	h.logger.Info("Handling disk stats request", "client_ip", c.ClientIP())
	...
	if err != nil {
		h.logger.Error("Failed to fetch historical disk metrics", "error", err, "hours", hours)
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}

	h.logger.Info("Disk stats response sent successfully", "history_points", len(historyMetrics))
