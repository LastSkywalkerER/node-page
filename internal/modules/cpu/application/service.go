...
type Service interface {
	...
	Save(ctx context.Context, metric entities.CPUMetric) error
...
func (s *service) Save(ctx context.Context, metric entities.CPUMetric) error {
	s.logger.Info("Saving CPU metrics to repository", "usage", metric.UsagePercent)
	err := s.cpuRepository.SaveCurrentMetric(ctx, metric)
	if err != nil {
		s.logger.Error("Failed to save CPU metrics", "error", err)
		return err
	}
	s.logger.Info("CPU metrics saved successfully")
	return nil
}
