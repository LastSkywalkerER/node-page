...
func (h *DockerHandler) HandleDockerStats(c *gin.Context) {
	h.logger.Info("Handling Docker stats request", "client_ip", c.ClientIP())
	...
	// Get historical Docker metrics
	historyMetrics, err := h.service.GetHistorical(c.Request.Context(), hours)
	if err != nil {
		h.logger.Error("Failed to fetch historical Docker metrics", "error", err, "hours", hours)
		c.JSON(http.StatusInternalServerError, gin.H{
			"error":            err.Error(),
			"docker_available": false,
		})
	...
	h.logger.Info("Docker stats response sent successfully", "total_containers", latestMetrics.TotalContainers, "running_containers", latestMetrics.RunningContainers, "history_points", len(historyMetrics))
