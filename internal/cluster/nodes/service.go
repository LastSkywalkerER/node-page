...
type Service interface {
	...
	Join(ctx context.Context, token string, hostInfo hosts.HostInfo) (hostID uint, nodeAccessToken string, err error)
...
type service struct {
	logger        *log.Logger
	joinTokenRepo JoinTokenRepository
...
func (s *service) Join(ctx context.Context, token string, hostInfo hosts.HostInfo) (hostID uint, nodeAccessToken string, err error) {
	if token == "" {
...
// HandlePush updates last_seen and agent_session_started_at (new session if gap > hosts.AgentPushGapSessionReset).
...
func (s *service) HandlePush(ctx context.Context, hostID uint, hostName, hostIPv4 string) error {
	...
	if host.AgentSessionStartedAt == nil || gap > hosts.AgentPushGapSessionReset {
		sessionStart = now
	} else {
...
func (s *service) UpdateAgentClusterConfig(mainNodeURL, nodeAccessToken string) error {
	...
	return Update(mainNodeURL, nodeAccessToken)
}
...
func (s *service) DeleteRemoteHost(ctx context.Context, hostID, currentHostID uint) error {
	...
	if hostID == hosts.LocalCollectorHostID {
		return ErrCannotDeleteLocalHost
	}
	if _, err := s.hostRepo.GetHostByID(ctx, hostID); err != nil {
		return err
