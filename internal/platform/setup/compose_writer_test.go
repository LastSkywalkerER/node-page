...
func TestBuildComposeContent_PostgresManaged(t *testing.T) {
	...
	mustContain(t, out, []string{
		...
		"mem_limit: 768m",
		"shm_size: 256mb",
		"command: postgres -c shared_buffers=${NODE_STATS_PG_SHARED_BUFFERS:-64MB} -c max_connections=${NODE_STATS_PG_MAX_CONNECTIONS:-16} -c wal_compression=on -c checkpoint_completion_target=0.9",
	})
}
