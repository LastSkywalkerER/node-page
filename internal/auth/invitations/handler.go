...
// Handler handles invitation-related HTTP requests.
type Handler struct {
	service Service
}
...
func (h *Handler) CreateInvitation(c *gin.Context) {
	...
	token, link, err := h.service.CreateInvitation(c.Request.Context(), adminID, baseURL, strings.TrimSpace(strings.ToLower(req.Email)))
	if err != nil {
...
func (h *Handler) ValidateInvitation(c *gin.Context) {
	token := c.Query("token")
	if token == "" {
		c.JSON(http.StatusBadRequest, gin.H{
			"code":  "validation_error",
	...
	inv, err := h.service.ValidateToken(c.Request.Context(), token)
	if err != nil || inv == nil {
		c.JSON(http.StatusBadRequest, gin.H{
