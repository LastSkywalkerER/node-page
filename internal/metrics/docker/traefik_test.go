...
func TestParseTraefikDirs_GatewayPortHint(t *testing.T) {
	dir := t.TempDir()
	content := "# Generated by node-stats (gateway)\n# node-stats-gateway: http_port=18080 https_port=18443\n" +
		"http:\n  routers:\n    ns-a:\n      rule: Host(`a.example.com`)\n      service: ns-a\n      entryPoints: [web]\n" +
		"    ns-b:\n      rule: Host(`b.example.com`)\n      service: ns-a\n      entryPoints: [websecure]\n      tls: {}\n" +
		"  services:\n    ns-a:\n      loadBalancer:\n        servers:\n        - url: http://host.docker.internal:19091\n"
	if err := os.WriteFile(filepath.Join(dir, "node-stats.yml"), []byte(content), 0o644); err != nil {
		t.Fatal(err)
	}
	routes := parseTraefikDirs([]string{dir})
	...
	if got["a.example.com"] != "http://a.example.com:18080" || got["b.example.com"] != "https://b.example.com:18443" {
		t.Fatalf("public urls with gateway ports: %v", got)
	}
	c := &DockerContainer{Name: "x", Ports: []DockerPort{{PrivatePort: 9090, PublicPort: 19091}}}
	if !routeMatchesContainer(routes[0], c) {
